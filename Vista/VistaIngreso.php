...
<!-- Panel similar al de usuarios/productos -->
<div class="panel mt-4">
    <div class="panel-header">
        <h5 class="panel-title">
            <i class="fas fa-truck-loading"></i> Ingresos de Productos
        </h5>
    </div>
        ...
        <div class="table-responsive">
            <table class="table table-bordered text-center align-middle">
                <thead class="table-dark">
                    <tr>
                        <th><i class="fas fa-receipt fa-sm"></i> No° Entrada</th>
                        <th><i class="fas fa-calendar-alt fa-sm"></i> Fecha de Ingreso</th>
                        ...
                        <th><i class="fas fa-dollar-sign fa-sm"></i> Valor de Ingreso</th>
                ...
                <tbody>
                    ...
                    if (isset($resultado) && $resultado->num_rows > 0) {
                        $fechaAnterior = null; // para comparar fechas
                        while ($fila = mysqli_fetch_assoc($resultado)) { 

                            $fechaActual = $fila['FechaIngresoProducto'];

                            // Color fila según si la fecha cambia o no
                            if ($fechaAnterior === null) {
                                // Primera fila
                                $colorFila = '#e3f2fd';  // azul claro (fecha nueva)
                            } elseif ($fechaActual === $fechaAnterior) {
                                $colorFila = '#ffffff';  // blanco (misma fecha)
                            } else {
                                // Fecha diferente a la anterior
                                $colorFila = '#e3f2fd';  // azul claro (fecha nueva)
                            }

                            $fechaAnterior = $fechaActual;

                            // Definir color Precio Total
                            $precioTotal = $fila['PrecioTotal'];
                            if ($precioTotal >= 1000000) {  // alto
                                $colorPrecio = '#f8d7da'; // rojo claro
                                $colorTexto = '#842029';
                            } elseif ($precioTotal >= 500000) {  // medio
                    ...
                    <?php 
                        ...
                        echo '<tr><td colspan="8" class="text-muted">No hay registros.</td></tr>';
                    }
                    ?>
                </tbody>
            </table>
    ...
    <div class="modal-dialog modal-sm" style="max-width: 40%; margin: 1rem auto;">
        <div class="modal-content rounded-3 shadow-lg" style="background: rgba(0, 0, 0, 0.6); color: #fff;">
            ...
            <div class="modal-body">
                ...
                <div class="mb-3 d-flex align-items-center">
  <label for="producto" class="form-label" style="color: #ffffff; font-size: 0.9rem; width: 40%;">Producto</label>
